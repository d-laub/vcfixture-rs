...
At 1 worker there is no parallelism benefit to factor out — this isolates whatever the pipeline-shape change (gap-only span pass + moving encode/summary into a single interleaved pass) is worth on its own. Result: **1.31x speedup, a 23.6% reduction in s/cell** (3.263e-7 / 4.272e-7 = 0.764). This is real and single-threaded, but it is well short of "roughly halves" (a 50% reduction, 2x). It is also not a pure "generation CPU" number — it reflects the combined effect of removing the redundant generation pass *and* restructuring encode/summary into the same pass, since both changes affect single-threaded work, not just generation.

Verdict: **did not hold as stated.** There is a genuine, directly measured single-threaded improvement (~1.31x / ~24% reduction), so the underlying change is not worthless outside of parallelism — but the magnitude falls well short of "roughly halves," and the harness still cannot isolate generation-only CPU from the combined pipeline-shape effect. Do not read the ~1.31x single-threaded figure, or the 2.5–3.1x multi-threaded speedup, as confirmation of a ~2x generation-CPU halving.
